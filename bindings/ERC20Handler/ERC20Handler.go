...
func (_ERC20Handler *ERC20HandlerCaller) GetDepositRecord(opts *bind.CallOpts, depositNonce uint64, destId uint8) (ERC20HandlerDepositRecord, error) {
	var out []interface{}
	err := _ERC20Handler.contract.Call(opts, &out, "_depositRecords",  destId, depositNonce)
	...
	record :=  new(ERC20HandlerDepositRecord);
	record.TokenAddress = *abi.ConvertType(out[0], new(common.Address)).(*common.Address)
	record.LenDestinationRecipientAddress = out[1].(uint8)
	record.DestinationChainID = out[2].(uint8)
	record.ResourceID = out[3].([32]byte)
	record.DestinationRecipientAddress = out[4].([]byte)
	record.Depositer = out[5].(common.Address)
	record.Amount = out[6].(*big.Int)

	//out0 := *abi.ConvertType(out, new(ERC20HandlerDepositRecord)).(*ERC20HandlerDepositRecord)
